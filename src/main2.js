...
rgbeLoader.load(
    'https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/1k/rural_asphalt_road_1k.hdr', // Replace with your HDRI path
    function (texture) {
...
loader.load(
    'public/scene.gltf', // Replace with your model path
    function (gltf) {
        model = gltf.scene;
        model.scale.set(50, 50, 50);
        mesh.add(model);

        camera.position.set(1, 1.1, 1.5); // X (left-right), Y (up-down), Z (forward-backward)
        camera.lookAt(model.position); // Make camera focus on the model
